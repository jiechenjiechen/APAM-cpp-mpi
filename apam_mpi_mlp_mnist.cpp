// Two-layer feedforward neural network on MNIST data set.
...
int main(int argc, char **argv) {
  ...
  std::string debug_sparse_outfile = "debug_mlp_sparse.txt";
  ...
  std::string debug_time_outfile = "debug_mlp_time.txt";
  bool debug_grad = false;   // if true, debugging info will print to file
  std::string debug_grad_outfile = "debug_mlp_grad.txt";
  ...
  MLP model;                              // move net to device
  model.to(device);
  APAM<MLP> optimizer(model, lr, use_amsgrad, use_sparse); // set optimizer
